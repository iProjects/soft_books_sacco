...
{
    public partial class EditCustomizableFieldsForm : Form
    {
        Repository rep;
        ...
        DAL.CustomizableFieldsModel _cust;

        public EditCustomizableFieldsForm(DAL.CustomizableFieldsModel cust,string Conn)
        {
            InitializeComponent();
        ...
        private void EditCustomizableFieldsForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                var _AdvancedFieldsquery = from et in rep.GetAdvancedFieldsWithCollectionItems(_cust.entity_id)
                                           select et;
                _lstadvancedfields = _AdvancedFieldsquery.ToList();
        ...
        private void btnDeleteField_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            if (dataGridViewAdvancedFields.SelectedRows.Count != 0)
            {
                try
                {
                    ...
                    var _advancedfieldsquery = from br in rep.GetAdvancedFieldValues(_advfld.advfieldsid)
                                               select br;
                    List<AdvancedFieldsValuesModel> _advancedfields = _advancedfieldsquery.ToList();
                    if (_advancedfields.Count > 0)
                    {
                        MessageBox.Show("There is a Client with Values Associated for this Field\nDelete the Values first!", "SB Sacco", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                        switch (_advfld.type_id)
                        {
                            case 5:
                                if (DialogResult.Yes == MessageBox.Show("This field contains collection.By clicking yes you will be deleting all collections Associated with this Field.Are you sure you want to delete Field " + _advfld.name.ToUpper().Trim(), "Confirm Delete", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question))
                                {

                                    _lstadvancedfields.Remove(_advfld);
                                    rep.DeleteAdvancedField(_advfld);
                                    RefreshGrid();
                                }
                                break;
                            default:
                                if (DialogResult.Yes == MessageBox.Show("Are you sure you want to delete Field\n" + _advfld.name.ToUpper().Trim(), "Confirm Delete", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question))
                                {
        ...
        private void InitializeControls()
        {
            ...
            { 
                if (_cust != null)
                {
                    AdvancedFieldsEntitiesModel afe = rep.GetAdvancedFieldEntity(_cust.entity_id);
                    cboAdvancedFieldEntities.SelectedValue = afe.advfieldsentitiesid;
                    cboAdvancedFieldEntities.Enabled = false;
                } 
            }
            catch (Exception ex)
            {
        ...
        public void DisableControls()
        {
            ...
            {
                cboAdvancedFieldEntities.Enabled = false; 
                txtattFieldDescription.Enabled = false;
                chkattMandatory.Enabled = false;
                chkattUnique.Enabled = false;
                txtFieldName.Enabled = false;
                txtDescription.Enabled = false;
                cboAdvancedFieldTypes.Enabled = false;
                chkMandatory.Enabled = false;
        ...
        private bool IsAdvancedFieldValid()
        {
            bool noerror = true;
            if (cboAdvancedFieldEntities.SelectedIndex == -1)
            {
