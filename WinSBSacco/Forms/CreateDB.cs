...
namespace WinSBSacco
{
    ...
    {
        const string DatabaseVersionExtPropertyKey = "SBSaccoVersion";
        const string DatabaseVersionExtPropertyValue = "1.0.0.0";
        const string version = @"IF NOT EXISTS (SELECT * FROM ::fn_listextendedproperty(N'SBSaccoVersion' , NULL,NULL, NULL,NULL, NULL,NULL)) 
                               EXEC dbo.sp_addextendedproperty @name=N'SBSaccoVersion', @value=N'1.0.0.0' 
                               GO";
        ...
        private void CreateDB_Load(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("Going to look for all SQL Servers in the Network this might take some time.\n Do you want to proceed...", "SB Sacco", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                {
                ...
                StreamReader sr = new StreamReader("Data/SBSaccoDB.sql");
