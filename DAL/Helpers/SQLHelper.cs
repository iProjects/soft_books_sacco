...
namespace DAL
{
    public partial class SQLHelper
    {
        ...
        static public string EntityConnection(SBSystem system, string userId, string Pwd, bool IntegratedSec)
        {
            ...
            string metadata = @"res://*/SBSaccoDBEntities.csdl|res://*/SBSaccoDBEntities.ssdl|res://*/SBSaccoDBEntities.msl";
            metadata.Replace("SBSaccoDBEntities", system.Metadata);
        ...
        static public void ScriptDatabase(string txtExportToFile, string ServerName, string DatabaseName)
        {
            ...
            try
            {
                ...
                // Start by manually adding DB db
            ...
            try
            {
                ...
                System.Windows.Forms.MessageBox.Show("DB exported to script at: " + txtExportToFile);
            }
            catch (Exception ex)
            {
        ...
        static public List<StoredProceduresModel> GetStoredProcedures(SBSystem system)
        {
            List<StoredProceduresModel> storedprocs = new List<StoredProceduresModel>();
            var srv = new Server(system.Server);

            Database db = srv.Databases[system.Database];
            // Iterate through the stored procedures in database 
            foreach (StoredProcedure sp in db.StoredProcedures)
            {
                if (sp.IsSystemObject == false)
                {
                    // Procedure is not a system object, so add it.
                    storedprocs.Add(new StoredProceduresModel
                    {
                        storedprocedureid = sp.ID,
                        Name = sp.Name,
                        Schema = sp.Schema,
                        TextHeader = sp.TextHeader,
